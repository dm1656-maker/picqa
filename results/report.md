...
_Generated 2026-05-14 16:24:56 by picqa._

## Inventory

- Source directory: `C:\Users\kdm16_korqogl\PycharmProjects\picqa\data`
- Total XML files: **709** (484.9 MB)
- Files per wafer: HY202103=709
...
## FWHM and Q-factor analysis

FWHM (Full Width at Half Maximum) measures the spectral width of a transmission peak at the -3 dB level (= 50 % linear intensity). The Q-factor (= λ_centre / FWHM) is dimensionless and quantifies how spectrally selective the device is — higher is sharper. The polynomial envelope of the grating coupler is subtracted before measurement so peaks appear flat near 0 dB.

### Per-wafer FWHM / Q summary

| Wafer | Band | FWHM_nm_count | FWHM_nm_median | FWHM_nm_std | Q_factor_count | Q_factor_median | Q_factor_std |
|---|---|---|---|---|---|---|---|
| D07 | C | 14 | 6.97 | 0.17 | 14 | 222.4 | 4.82 |
| D08 | C | 14 | 7.05 | 0.17 | 14 | 220.3 | 4.93 |
| D08 | O | 14 | 4.86 | 0.12 | 14 | 269.2 | 6.53 |
| D23 | O | 28 | 4.79 | 0.15 | 28 | 272.8 | 8.16 |
| D24 | O | 28 | 4.81 | 0.14 | 28 | 273 | 8.24 |

## Cross-parameter efficiency analysis

Each parameter is normalised to a 0–1 score (1 = best in this dataset) and combined with the default weights (Vπ ×2, ER ×1.5, IL ×1, leakage ×1, FSR ×0.5) into a single **EfficiencyScore** per die. Scoring is robust to a few outliers (uses the 5–95th percentile range), and dies with NaN metrics get the remaining weights re-distributed.

### Top dies overall

| Wafer | Die | Band | Vpi_V | ER_at_-2V_dB | PeakIL_dB | I_at_-1V_pA | EfficiencyScore |
|---|---|---|---|---|---|---|---|
| D08 | (-1,2) | O | 25.3 | 32.78 | -6.363 | 3.537e+04 | 0.8011 |
| D08 | (2,2) | O | 25.08 | 32.82 | -7.366 | 3.945e+04 | 0.7991 |
| D08 | (2,0) | O | 25.26 | 29.52 | -7.007 | 3.444e+04 | 0.7547 |
| D08 | (-1,-4) | C | 28.94 | 37.52 | -9.849 | 2718 | 0.7451 |
| D08 | (2,-3) | O | 26.62 | 33.24 | -8.631 | 5.938e+04 | 0.7404 |
| D07 | (0,-3) | C | 27.85 | 39.46 | -10.05 | 4.815e+04 | 0.7357 |
| D08 | (-4,0) | O | 26.44 | 32.92 | -8.328 | 7.636e+04 | 0.7286 |
| D08 | (3,-1) | O | 22.79 | 31.49 | -9.008 | 5.357e+04 | 0.7221 |
| D08 | (0,3) | O | 28.96 | 31.81 | -6.863 | 5.3e+04 | 0.7193 |
| D08 | (-3,2) | O | 26.51 | 29.39 | -7.743 | 6.346e+04 | 0.7114 |

### Position summary (mean efficiency by region)

| category | level | n | mean | median |
|---|---|---|---|---|
| Region | center | 16 | 0.546 | 0.555 |
| Region | edge | 44 | 0.561 | 0.593 |
| Quadrant | NE | 16 | 0.581 | 0.605 |
| Quadrant | NW | 14 | 0.561 | 0.533 |
| Quadrant | SW | 17 | 0.501 | 0.526 |
| Quadrant | SE | 13 | 0.595 | 0.594 |
| Radius | r=0 | 4 | 0.521 | 0.521 |
| Radius | r=1 | 4 | 0.497 | 0.477 |
| Radius | r=2 | 8 | 0.582 | 0.61 |
| Radius | r=3 | 20 | 0.575 | 0.614 |
| Radius | r=4 | 24 | 0.549 | 0.557 |
...
**Interpretation:** these die positions consistently produce devices in the upper quartile of efficiency across multiple wafers, suggesting that the process favours these locations. Keeping such dies preferentially during binning would raise overall yield.

### Combined sweet spots — strong on multiple metrics

Sweet-spot analysis was repeated separately for EfficiencyScore, Q-factor, FWHM, and Vπ. The table below lists positions that landed in the top tier on **two or more axes** — these are the strongest multi-criteria candidates because no single metric is dominating the decision.

| DieCol | DieRow | n_axes | axes_str | total_n_wafers_top |
|---|---|---|---|---|
| 0 | -3 | 2 | EfficiencyScore+Vpi_V | 6 |
| -3 | -3 | 2 | FWHM_nm+Q_factor | 6 |
| 2 | -3 | 2 | EfficiencyScore+Q_factor | 5 |
| -1 | -1 | 2 | FWHM_nm+Q_factor | 4 |
| 2 | 2 | 2 | FWHM_nm+Q_factor | 4 |
| -4 | 0 | 2 | FWHM_nm+Q_factor | 4 |
| -3 | 2 | 2 | FWHM_nm+Q_factor | 4 |

**Interpretation:** positions tagged `Eff+Vπ` are good for variability AND modulation strength. Positions tagged `FWHM+Q` are good in spectral selectivity but may be average on overall efficiency (since Q and FWHM measure the same physical property). Positions matching `Eff+Q` are the most robust binning candidates because they combine independent quality axes.
...
### FWHM annotated illustration
![FWHM](figures\fwhm_annotated.png)

### Q-factor distribution across wafers
![Q distribution](figures\q_factor_distribution.png)

### FWHM and Q-factor wafer maps
![FWHM wafermap](figures\fwhm_wafermap.png)

### Cross-parameter efficiency map
![Efficiency map](figures\efficiency_wafermap.png)

### Sweet-spot map
![Sweet spots](figures\sweet_spots.png)

### Per-metric sweet-spot maps
![Multi-metric sweet spots](figures\multi_metric_sweet_spots.png)

### Combined multi-axis sweet spots
![Combined sweet spots](figures\combined_sweet_spots.png)

## Per-wafer representative-die figures

Each wafer (and band, where applicable) gets its own subfolder under `figures/` containing the full plot set for the die closest to (0, 0) with a healthy contact. This gives a quick side-by-side reference die per wafer.

### D07 (C-band)  -  대표 die (0,0)
디바이스: `MZMCTE_LULAB_450_500`

**IV (semilog)**
![IV (semilog)](figures\D07_C\iv.png)

**Transmission spectra**
![Transmission spectra](figures\D07_C\spectra.png)

**Bias-shift spectra**
![Bias-shift spectra](figures\D07_C\bias_shift.png)

**V-λ (변조 효율 nm/V)**
![V-λ (변조 효율 nm/V)](figures\D07_C\v_lambda.png)

**V-φ**
![V-φ](figures\D07_C\vphi.png)

**6패널 V-π·L 분석**
![6패널 V-π·L 분석](figures\D07_C\vpi_analysis.png)

### D08 (C-band)  -  대표 die (2,-3)
디바이스: `MZMCTE_LULAB_450_500`

**IV (semilog)**
![IV (semilog)](figures\D08_C\iv.png)

**Transmission spectra**
![Transmission spectra](figures\D08_C\spectra.png)

**Bias-shift spectra**
![Bias-shift spectra](figures\D08_C\bias_shift.png)

**V-λ (변조 효율 nm/V)**
![V-λ (변조 효율 nm/V)](figures\D08_C\v_lambda.png)

**V-φ**
![V-φ](figures\D08_C\vphi.png)

**6패널 V-π·L 분석**
![6패널 V-π·L 분석](figures\D08_C\vpi_analysis.png)

### D08 (O-band)  -  대표 die (0,0)
디바이스: `MZMOTE_LULAB_380_500`

**IV (semilog)**
![IV (semilog)](figures\D08_O\iv.png)

**Transmission spectra**
![Transmission spectra](figures\D08_O\spectra.png)

**Bias-shift spectra**
![Bias-shift spectra](figures\D08_O\bias_shift.png)

**V-λ (변조 효율 nm/V)**
![V-λ (변조 효율 nm/V)](figures\D08_O\v_lambda.png)

**V-φ**
![V-φ](figures\D08_O\vphi.png)

**6패널 V-π·L 분석**
![6패널 V-π·L 분석](figures\D08_O\vpi_analysis.png)

### D23 (O-band)  -  대표 die (0,0)
디바이스: `MZMOTE_LULAB_380_500`

**IV (semilog)**
![IV (semilog)](figures\D23_O\iv.png)

**Transmission spectra**
![Transmission spectra](figures\D23_O\spectra.png)

**Bias-shift spectra**
![Bias-shift spectra](figures\D23_O\bias_shift.png)

**V-λ (변조 효율 nm/V)**
![V-λ (변조 효율 nm/V)](figures\D23_O\v_lambda.png)

**V-φ**
![V-φ](figures\D23_O\vphi.png)

**6패널 V-π·L 분석**
![6패널 V-π·L 분석](figures\D23_O\vpi_analysis.png)

### D24 (O-band)  -  대표 die (0,0)
디바이스: `MZMOTE_LULAB_380_500`

**IV (semilog)**
![IV (semilog)](figures\D24_O\iv.png)

**Transmission spectra**
![Transmission spectra](figures\D24_O\spectra.png)

**Bias-shift spectra**
![Bias-shift spectra](figures\D24_O\bias_shift.png)

**V-λ (변조 효율 nm/V)**
![V-λ (변조 효율 nm/V)](figures\D24_O\v_lambda.png)

**V-φ**
![V-φ](figures\D24_O\vphi.png)

**6패널 V-π·L 분석**
![6패널 V-π·L 분석](figures\D24_O\vpi_analysis.png)
